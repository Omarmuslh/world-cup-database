...
echo -e "\nYear and team name of all teams that scored two goals in a round:"
echo $($PSQL "SELECT year, name FROM games JOIN teams ON (games.winner_id = teams.team_id OR games.opponent_id = teams.team_id) WHERE winner_goals = 2 OR opponent_goals = 2 ORDER BY year")

echo -e "\nList of unique teams that played in the 2014 'Eighth Final' round:"
